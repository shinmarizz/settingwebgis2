//#region node_modules/@terraformer/wkt/dist/t-wkt.esm.js
/* @preserve
...
PointArray.prototype.addPoint = function(point) {
	if (point.type === "PointArray") this.data = this.data.concat(point.data);
...
Ring.prototype.toJSON = function() {
	var data = [];
	for (var i = 0; i < this.data.data.length; i++) data.push(this.data.data[i]);
	return data;
};
...
RingList.prototype.toJSON = function() {
	var data = [];
	for (var i = 0; i < this.data.length; i++) data.push(this.data[i].toJSON());
...
PolygonList.prototype.toJSON = function() {
	var data = [];
	for (var i = 0; i < this.data.length; i++) data = data.concat([this.data[i].toJSON()]);
	return data;
};
/**
...
var arrayToRing = function arrayToRing(arr) {
	var parts = [];
	var ret = "";
	for (var i = 0; i < arr.length; i++) parts.push(arr[i].join(" "));
	ret += "(" + parts.join(", ") + ")";
	return ret;
};
var pointToWKTPoint = function pointToWKTPoint(geojson) {
	var ret = "POINT ";
	if (geojson.coordinates === void 0 || geojson.coordinates.length === 0) {
		ret += "EMPTY";
		return ret;
	} else if (geojson.coordinates.length === 3) if (geojson.properties && geojson.properties.m === true) ret += "M ";
	else ret += "Z ";
	else if (geojson.coordinates.length === 4) ret += "ZM ";
	ret += "(" + geojson.coordinates.join(" ") + ")";
	return ret;
};
var lineStringToWKTLineString = function lineStringToWKTLineString(geojson) {
	var ret = "LINESTRING ";
	if (geojson.coordinates === void 0 || geojson.coordinates.length === 0 || geojson.coordinates[0].length === 0) {
		ret += "EMPTY";
		return ret;
	} else if (geojson.coordinates[0].length === 3) if (geojson.properties && geojson.properties.m === true) ret += "M ";
	else ret += "Z ";
	else if (geojson.coordinates[0].length === 4) ret += "ZM ";
...
var polygonToWKTPolygon = function polygonToWKTPolygon(geojson) {
	...
	if (geojson.coordinates === void 0 || geojson.coordinates.length === 0 || geojson.coordinates[0].length === 0) {
		ret += "EMPTY";
		return ret;
	} else if (geojson.coordinates[0][0].length === 3) if (geojson.properties && geojson.properties.m === true) ret += "M ";
	else ret += "Z ";
	else if (geojson.coordinates[0][0].length === 4) ret += "ZM ";
	ret += "(";
	var parts = [];
	for (var i = 0; i < geojson.coordinates.length; i++) parts.push(arrayToRing(geojson.coordinates[i]));
	ret += parts.join(", ");
	ret += ")";
	return ret;
};
...
var multiPolygonToWKTMultiPolygon = function multiPolygonToWKTMultiPolygon(geojson) {
	...
	if (geojson.coordinates === void 0 || geojson.coordinates.length === 0 || geojson.coordinates[0].length === 0) {
		ret += "EMPTY";
		return ret;
	} else if (geojson.coordinates[0][0][0].length === 3) if (geojson.properties && geojson.properties.m === true) ret += "M ";
	else ret += "Z ";
	else if (geojson.coordinates[0][0][0].length === 4) ret += "ZM ";
	ret += "(";
	var inner = [];
	for (var c = 0; c < geojson.coordinates.length; c++) {
		var it = "(";
		var parts = [];
		for (var i = 0; i < geojson.coordinates[c].length; i++) parts.push(arrayToRing(geojson.coordinates[c][i]));
		it += parts.join(", ");
		it += ")";
		inner.push(it);
	}
...
var geojsonToWKT = function geojsonToWKT(geojson) {
	...
	var parts = [];
	switch (geojson.type) {
		case "Point": return pointToWKTPoint(geojson);
		case "LineString": return lineStringToWKTLineString(geojson);
		case "Polygon": return polygonToWKTPolygon(geojson);
		case "MultiPoint": return multiPointToWKTMultiPoint(geojson);
		case "MultiLineString": return multiLineStringToWKTMultiLineString(geojson);
		case "MultiPolygon": return multiPolygonToWKTMultiPolygon(geojson);
		case "GeometryCollection":
			for (var i = 0; i < geojson.geometries.length; i++) parts.push(geojsonToWKT(geojson.geometries[i]));
			return ret + "(" + parts.join(", ") + ")";
		default: throw Error("Unknown Type: " + geojson.type);
	}
};
//#endregion
